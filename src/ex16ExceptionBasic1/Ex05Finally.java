...
/*
 	...
 	: 예외발생과 상관없이 try문에 진입했을 때 반드시
 	실행해야할 코드가 있는 경우 기술하는 블럭이다.
 	즉 try문으로 진입하지 않는다면 finally절은 실행되지 않는다.
 */
public class Ex05Finally {
	/*
	 런타임 예외가 발생되는 메소드. 예외 발생시 무시하기 위해
	 throws하고 있다. 하지만 이와같은 예외는 어디서든 반드시
	 예외 처리해야한다.
	 */
	...
	static void tryCatchFinally() {
		...
		try {
			...
			/*
			 try문 진입 후 예외 발생 여부에 상관없이 해당 구문은 실행된다.
