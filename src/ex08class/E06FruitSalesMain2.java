package ex08class;
/*
 FruiteSalesMain 이전 버전은 각 클래스가 멤버변수의 초기 값이 고정되어, 
 상태가 다른 객체를 생성할 수 없는 단점이 있었다.
 두번재 버전에서는 다양한 초기값을 부여할 수 있도록 초기화 메서드를 도입해본다.
...
class FruitBuyer2{
		...
		//초기화 메소드 선언
		public void initMembers(int _myMoney, int _numOfApple){ //다른 클래스의 같은 메서드 이름은 다른 친구임.
			myMoney = _myMoney;
...
{	
	...
	{
		/*
		 초기화 메서드가 정의되었으므로 상태가 다른 객체를 생성할 수 잇다.
		 */
