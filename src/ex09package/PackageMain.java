...
/*
 ...
 Package Main클래스는 ex09package 패키지 하위에
 생성된다.
 */
package ex09package;
...
/*
 ...
 함께 작성하면 된다.
 만약 해당 패키지 하위에 선언된 모든 클래스를 한꺼번에 임포트 하고 싶다면
 import java.util*와 같이 와일드 카드를 사용하면 된다.  (* = 모든것을 뜻함)
 */

import java.util.Date; //util package의 Date를 import했다는 뜻.
/*import java.sql.Date;를 하게되면 밑에 Date 객체가 util에서 삽입된것인지,
  sql에서 삽입된것인지를 알 수 없어 애매해서 에러가 나온다.
*/
...
public class PackageMain
{
	...
	public static void main(String[] args)
	{
		...
		//패키지의 풀경로를 사용
		java.sql.Date sqlDate =  //얘는 풀경로를 명시함으로써 util의 Date가 아니라 sql의 Date라는것을 알리는 것임.
				new java.sql.Date(utilDate.getTime());
		System.out.println("sqlDate= " + sqlDate);
		
		/*
		 ex09package 하위에 ex09package.kosmo.util 패키지가 생성되지만
		 이 둘은 서로 다른 패키조로 인식한다.
		 따라서 isNumber()메서드를 호출하기 위해 반드시 해당패키지를
		 import해야 한다.
		 */
		String strValue = "987654321";
		/*
		 static으로 선언된 메서드는 new를 통한 개체 생성없이
		 클래스명으로 직접 호출할 수 잇다.
		 */
