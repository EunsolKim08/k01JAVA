...
public class E01WrapperClass2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("10+strNumber를 숫자로 변경: " + //숫자+ 슷지
				(10+Integer.parseInt(strNumber)));
		System.out.println("10+strNumber를 숫자로 변경: " +  //숫자+숫자
		...
		/*
		 실수형태의 문자열을 실수로 변경할 대는 parseInt()를 사용할 수 없다.
		 정수가 아니기 때문. 위와 동일한 예외가 발생한다.
		 */
		//System.out.println(Integer.parseInt(floatString));
		/*
		 codePointAt(): 해당 인덱스의 아스키코드 반환
		 */
		System.out.println("실수형(float형으로 변경:" );
			Float.parseFloat(floatString);
		System.out.println("실수형(Double형으로 변경:" );
		 ...
		 */
		/*
		 isDigit(): 인자로 전달된 문자가 숫자인지 판단. 숫자일경우 true반환.
		 */
		...
		/*
		 isLetter(): 문자인지 여부를 판단하는 메서드로 특수기호나 숫자형일 때는 
		 false를 반환한다.
		 */
		 ...
		 */
		System.out.println("isWhitespace()메소드를 통한 공백문자 판단");
		...
		/*
		 시나리오] 주어진 문자열안에 몇개의 공백문자(스페이스)가 있는지 판단하는
		 프로그램을 작성하시오.
		 
		 	참고)toCharArray() : 문자열을 char 형 배열로 반환해주는
		 	String클래스에 정의된 메소드
		 */
