package ex18thThread;
/*
 Thread(쓰레드)
 -쓰레드의 생성은 Thread클래스를 상속받는 것부터 시작한다.
 - 해당방법은 쓰레드로 생성할 클래스가 다른 클래스를 상속받지
 않아도 될때 사용하는 방식이다.
 -만약 해당클래스가 다른 클래스를 상속해야한다면 Ruunable
 인터페이스를 구현하여 사용한다.
 */
class ShowThread extends Thread{
	
	String threadName;
	
	public ShowThread(String threadName) {
		this.threadName = threadName;
	}
	/*
	 run() 메서드는 쓰레드 main() 메서드에 해당한다.
	 Thread 클래스의 run() 메서드를 오버라이딩 한 것으로 해당 메서드는
	 직접 호출하면 안되고, start()메서드를 통해 간접적으로 호출해야 한다.
	 만약 지적홉출하면 단순한 실행만 될뿐 쓰레도가 생성되지 않는다.
	 */
	@Override
	public void run() {
		for(int i=1; i<=100;i++) {
			System.out.println("안녕하세요.[ "+threadName+ "]입니다.");
			try {
				//잠깐동안 블럭상태로 들어가게 만들어주는 메서드
				/*
				 Thread 클래스의 static메서드로 실행의 흐름을 주어진
				 시간만큼 block상태로 전환시킨다.
				 1000분의 1초 단위로 기술하게 되므로 아래는 0.01초만큼 block
				 시키게 된다. 즉 1000이 1초이다.
				 */
				sleep(10);
			}
			catch(InterruptedException e) { //InterruptedException은 sleep이 기본으로 있는거임
					e.printStackTrace();
			}
		}
	}
}

public class Ex01ThreadStart  {
	
	public static void main(String[] args) {
	
		//쓰레드 객체 생성
		ShowThread st1 = new ShowThread("쓰레드1st");
		ShowThread st2 = new ShowThread("Thread2nd");
		
		
		//쓰레드 객체를 통한 쓰레드 생성 및 실행 
		st1.start();
		st2.start();
		
		/*
		 run()메소드를 통한 호출은 단순한  호출한 단순한 
		 
		 */
	
	}
}
