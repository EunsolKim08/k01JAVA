...
접근방법
 	...
 	- 클래스 외부: 참조변수를 통한 접근 or 클래스 명을 통한 접근
 	 둘다 가능하다. 단, 클래스명으로 접근할 것을 권장한다.
 */
class MyStatic{
	/*
	 멤버변수]인스턴스형 멤버 변수는 반드시 클래스의 객체를 생선 한 후
	 참조변수를 통해 접근해야한다.
	 */
	int instanceVar;
	/*
	 정적 멤버변수]객체를 새엇ㅇ할 필요없이 클래스명으로 바로 접근할 수 있다.
	 프로그램 시작시 이미 메소드 영역에 로드된다
	 */
	static int staticVar;
	/*
	 인스턴스형 멤버메소드] 인스턴스형 메소드에서는 모든 멤버를 사용 할 수 있다.
	 인스턴스형 변수, 정적 변수 둘다 접근가능하다ㅣ
	 */
	void instacneceMethodD() {
		System.out.println("instanceVar= " + instanceVar);
		System.out.println("instanceVar= " + staticVar);
		staticMethod();
	}
	/*
	 정적멤버메소드] 정적 메소드 안에서는 인스턴스형 멤버를 사용할 수 없다.
	 정적 멤버만 접근할 수 있다.(main메소드에서는 static으로 선언된 메소드만
	 호출 할 수 있었음을 기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		/*
		 일반적인 멤버변수에 접근하는 방법: myStatic클래스의 객체를 생성한 후 참조변수
		 를 통해 접근한다.
		 */
		...
		/*
		 정적 멤버 메소드의접근 방법은 정적변수의 접근방법과 동일하다
		 */
		myStatic.instanceMethod();
		myStatic.staticMethod();//권장사항이 아니므로 클래스명 사용해라
