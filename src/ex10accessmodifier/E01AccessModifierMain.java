...
//		System.out.println("one.privateVar= "+ one.privateVar);
		...
		one.publicMethod();
		/*
		 private으로 선언된 멤버에 외부클래스에서 접근하려면 public으로 선언된 멤버
		 메서드를 통해 간접적으로 접근해야 한다.
		 publicMethod()를 통해 privateMethod()에 접근 할 수 잇다.
		 */
		/*
		 동일한 패키지에 선언된 클래스이므로 import없이 객체 생성이 가능하다.
		 또한 default클래스는 동일하게 접근가능
		 */
		...
		/*
		 다른 패키지에 선언된 클래스이므로 객체생성을 위해 import선언을 해야한다. 또한 패키지가 다르면
		 private, default멤버에 접근할 수 없다.
		 */
		...
		//two.defualtMethod(); 에러발생
		two.publicMethod();
		
		/*
		 다른 패키지에 선언된 디폴트 클래스이므로 객체생성이 불가능하다.
		 import도 불가능하므로 사용할 수 없는 클래스가 된다.
		 */
		System.out.println("DefaultClass 객체 생성 및 접근");
