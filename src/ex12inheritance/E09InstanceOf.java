...
//Box b라는거 자체가 Box타입이라는 뜻.
	/*
	 -여기서 instanceof 연산자를 통해 형변환 가능여부를 판단
	 하는 이유는 전달된 매개변수를 최상위 클래스인 Box타입으로
	 받고 있기 때문이다.
	 - Box 타입으로 인자를 받게되면 자동으로 업캐스팅(자동형변환)이
	 되기 대문에 자식클래스의 멤버메스드를 호출할 수 없게된다.
	 - 이때 자식의 멤버메소드를 호출하기 위해서 다운캐스팅(강제형변환)이
	 필요하다.
	 - 따라서 전달된 인자의 각각 타입으로 형변환이 가능한지 판단 후
	...
	public static void main(String[] args) {
		...
		//wrapBox(str); //String객체와는 상속관계가 없으므로 호출 불가능.
	}
